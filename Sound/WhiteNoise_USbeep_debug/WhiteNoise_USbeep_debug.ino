...
void loop() {
  ...
  if (sound_type==1){
    Serial.println("Type 1 (white noise)");
    while ((unsigned long)(millis() - initTime) < 1000){
     generateNoise(frequency);
//     SetGain();
    }
    delay(500);
    Serial.println("exit Type 1");
  }
  else if (sound_type > 1){
    Serial.println("Type 2 (beep)");
     // generate beep
    tone(RspeakerPin, pitch, 500);
    delay(500);
    Serial.println("exit Type 2");
  }
  sound_type=0;
}
...
unsigned long ReadInstruction() {
  ...
  while ((unsigned long)(millis() - initTime) < 1500){
...
//      Serial.println((unsigned long)(millis() - initTime));
      delay(100);
  }
  return highCounter;
}
