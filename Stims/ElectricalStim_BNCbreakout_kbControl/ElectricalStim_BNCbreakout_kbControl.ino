...
p - pause
 */
/* Stim parameters Isokawa et al 87: 
  0.01-0.2 mA at a frequency of 10 Hz for 10-40 s (pulse duration: 6ms).
*/
...
unsigned long pulseDuration = 6;           // pulse duration (ms)
unsigned long interPulseInterval = int(1000/pulseFrequency)-pulseDuration;          // ISI (ms)
unsigned long trainInterval = 1000 + (pulsesPerTrain*pulseDuration) + ((pulsesPerTrain-1)*interPulseInterval);         //delay between pulse sets (ms)
